...
def test_arithmetic():
    ...
    # Let's verify Permutation 0 (Input: 1, 2, 3)
    # x1 (idx 1) = 1
    # x2 (idx 2) = 2
    # x4 (idx 4) should be 1 + 2 = 3
    val_univ0 = obs[4]
    
    # Let's verify Permutation 1 (Input: 1, 3, 2)
    # x1 (idx 9) = 1
    # x2 (idx 10) = 3
    # x4 (idx 12) should be 1 + 3 = 4
    # Note: Permutation 1 starts at index 8. x4 is 8+4=12.
    val_univ1 = obs[12]
    
    print(f"Permutation 0 Result: {val_univ0} (Expected 3.0)")
    print(f"Permutation 1 Result: {val_univ1} (Expected 4.0)")

    if val_univ0 == 3.0 and val_univ1 == 4.0:
        print(" - SUCCESS: ADD Instruction verified across parallel Permutations.")
    else:
...
def test_cmov():
    ...
    obs = env.observe()
    # Permutation 0: x1=1, x2=2. Condition=1 (True). Result x6 should be 1.
    val = obs[6]
    
    print(f"CMOV Result (Cond True): {val} (Expected 1.0)")
    
    if val == 1.0:
        print(" - SUCCESS: CMOV verified.")
    else:
